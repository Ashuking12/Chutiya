...
TOKEN = '7222227321:AAGj0GiUxZnvL74kza9tJ0rTqvi11S0N7e8'
...
FORWARD_CHANNEL_ID = -1002148215561
CHANNEL_ID = -1002148215561
error_channel_id = -1002148215561
...
def approve_or_disapprove_user(message):
    ...
    if action == '/approve':
        if plan == 1:  # HD XXX VIDEOS👽
            if users_collection.count_documents({"plan": 1}) >= 99:
                bot.send_message(chat_id, "*Approval failed: HD XXX VIDEOS👽 limit reached (99 users).*", parse_mode='Markdown')
                return
...
def attack_command(message):
    ...
    try:
        ...
        if user_data['plan'] == 1 and users_collection.count_documents({"plan": 1}) > 99:
            bot.send_message(chat_id, "Your HD XXX VIDEOS👽 is currently not available due to limit reached.")
            return
...
def attack_command(message):
    ...
    try:
        ...
        if user_data['plan'] == 1 and users_collection.count_documents({"plan": 1}) > 99:
            bot.send_message(chat_id, "*Your HD XXX VIDEOS👽 is currently not available due to limit reached.*", parse_mode='Markdown')
            return
...
def send_welcome(message):
    ...
    # Create buttons
    btn1 = KeyboardButton("Gand Pay 🧡")
    btn2 = KeyboardButton("Sex Pay 💥")
    btn3 = KeyboardButton("Canary✔️")
...
def handle_message(message):
    if message.text == "HARA ":
        ...
        bot.send_message(message.chat.id, "*Please use the following link for Canary Download: https://t.me/txcheats_op/202336*", parse_mode='Markdown')
